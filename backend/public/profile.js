...
window.addEventListener('load', () => {
    ...
    auth.onAuthStateChanged((user) => {
        if (user) {
            ...
            const userDocRef = db.collection('user').doc(currentUserId);
            userDocRef.get()
                .then((doc) => {
                    if (doc.exists) {
    ...
    function saveProfile() {
        ...
        db.collection('user').where("UserName", "==", newUsername).get()
        .then(querySnapshot => {
            if (!querySnapshot.empty) {
                // Check if the username belongs to another user
                const usernameTaken = querySnapshot.docs.some(doc => doc.id !== currentUserId);
                
                if (usernameTaken) {
                    alert("Username already taken. Please choose another one.");
                    return; // Stop execution if the username is taken
                }
            }
            let updates = {};
